...
T* allocator<T, init_size>::allocate(size_t n) {
    ...
    if (n > init_size) {
        std::cerr << "ОШИБКА: Запрос " << n << " превышает размер блока "
                  << init_size << "\n";
        throw std::bad_alloc();
    }
        ...
        for (size_t i = 0; i < n; ++i) {
            if (index + i < capacity) {
                if (!free_slots[index + i]) {
                    ...
                    std::cerr << "ОШИБКА: Повторное освобождение по адресу "
                              << p << ", слот " << (index + i) << ", в блоке "
                              << block_id << "\n";
                    /*throw std::bad_alloc();*/
                }
            }
...
template <typename T, size_t init_size>
template <typename U>
bool allocator<T, init_size>::operator==(
    const allocator<U, init_size>&) const noexcept {
    return true;
}
...
template <typename U>
bool allocator<T, init_size>::operator!=(
    const allocator<U, init_size>& other) const noexcept {
    return !(*this == other);
}
...
std::ostream& operator<<(std::ostream& os, const allocator<T, N>& alloc) {
    os << "allocator[блок #" << reinterpret_cast<const void*>(&alloc)
       << ", data=" << alloc.get_data() << ", used=" << alloc.get_used() << "/"
       << alloc.get_capacity() << "]";
    return os;
}
...
MyContainer<T, Alloc>::Node::Node(const T& val)
        : value(val)
        , next(nullptr) {}
...
inline void
fill_custom_map(std::map<int, int, std::less<int>,
...
inline void
fill_my_container_with_alloc(MyContainer<int, allocator<int, 10>>& container) {
    for (int i = 0; i < 10; ++i) {
        container.add(i);
    }
}
...
void print_map(const MapType& map, const std::string& container_name = "") {
    ...
    std::cout << "Размер: " << map.size() << " элементов" << std::endl
              << std::endl;
}

template <typename ContainerType>
void print_container(const ContainerType& container,
                     const std::string& container_name = "") {
    std::cout << (container_name.empty() ? "Container" : container_name)
              << std::endl;
    std::cout << "Содержимое: ";

    container.print();

    std::cout << "Размер: " << container.size() << " элементов" << std::endl
              << std::endl;
}
}  // namespace common